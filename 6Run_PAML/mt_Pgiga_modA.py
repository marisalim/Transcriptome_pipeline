...
def ModelA_nullcodeml_Pgiga(element, kappastart):
	...
	cml.tree = '/pylon1/bi4iflp/mlim/Run_codeml_mtdna/transcriptometree_noroot_Pgiga.tree'
...
def ModelA_poscodeml_Pgiga(element, kappastart, omegastart):
	cml = codeml.Codeml()
	cml.alignment = '/pylon1/bi4iflp/mlim/Run_codeml_mtdna/mtDNA_phylipfiles/' + element + '.phylip'
	cml.tree = '/pylon1/bi4iflp/mlim/Run_codeml_mtdna/transcriptometree_noroot_Pgiga.tree'
	cml.out_file = element + '_Pgiga_modApos.out'
	cml.working_dir = './'
	...
	if '.phylip' in aname:
		...
		while notconverged:
			...
			if 'check convergence..' in open(file_null + '_Pgiga_modAnull.out').read(): 
				...
				os.system('mv ' + file_null + '_Pgiga_modAnull.out /pylon1/bi4iflp/mlim/Run_codeml_mtdna/mtDNA_codemlout_Pgiga/Pgigaout')
				print 'Success!!!', file_null, ' moved to Pgigaout directory'
				print '----------------------------------------------------------------'
				break
...
for aname in alignmentfiles:
	if '.phylip' in aname:
		...
		while notconverged:
			...
			ModelA_poscodeml_Pgiga(file_pos, kappa_starts[counter], omega_starts[counter])
	
			if 'check convergence..' in open(file_pos + '_Pgiga_modApos.out').read():
				print 'Nope, try again...', file_pos
				print '---------------------------------------'
				
			else: 
				os.system('mv ' + file_pos + '_Pgiga_modApos.out /pylon1/bi4iflp/mlim/Run_codeml_mtdna/mtDNA_codemlout_Pgiga/Pgigaout')
				print 'Success!!!', file_pos, ' moved to Pgigaout directory'
				print '----------------------------------------------------------------'
				break
...
os.system('grep lnL /pylon1/bi4iflp/mlim/Run_codeml_mtdna/mtDNA_codemlout_Pgiga/Pgigaout/*modAnull.out | awk \'{print $1"\t"$5}\' > /pylon1/bi4iflp/mlim/Run_codeml_mtdna/Pgiga_lnL_null.txt')
os.system('grep lnL /pylon1/bi4iflp/mlim/Run_codeml_mtdna/mtDNA_codemlout_Pgiga/Pgigaout/*pos.out | awk \'{print $1"\t"$5}\' > /pylon1/bi4iflp/mlim/Run_codeml_mtdna/Pgiga_lnL_pos.txt')
