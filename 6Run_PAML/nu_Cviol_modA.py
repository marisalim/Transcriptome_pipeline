	...
	if '.phylip' in aname:
		...
		while notconverged:
			...
			if 'check convergence..' in open(file_pos + '_Cviol_modApos.out').read():
				...
				os.system('mv ' + file_pos + '_Cviol_modApos.out /pylon1/bi4iflp/mlim/Run_codeml_nudna/nuDNA_codemlout_Cviol/Cviol_pos/Cviolout_nu_pos')
				print 'Success!!!', file_pos, ' moved to Cviolout_nu_pos directory'
				print '----------------------------------------------------------------'
				break
...
os.system('grep lnL /pylon1/bi4iflp/mlim/Run_codeml_nudna/nuDNA_codemlout_Cviol/Cviol_pos/Cviolout_nu_pos/*pos.out | awk \'{print $1"\t"$5}\' > /pylon1/bi4iflp/mlim/Run_codeml_nudna/nu_Cviol_lnL_pos.txt')
