...
def ModelA_poscodeml_Aamaz(element, kappastart, omegastart):
	...
	cml.tree = '/pylon1/bi4iflp/mlim/Run_codeml_nudna/transcriptometree_noroot_Aamaz.tree' #Note: using an unrooted tree
...
# FOR RERUNS:
alignmentfiles_pos = ['/pylon1/bi4iflp/mlim/Run_codeml_nudna/phylipforpaml/ENSTGUP00000011391.phylip']

# Choose start values
kappa_starts = [0, 1, 2, 5, 7, 20] # range of start values for kappa
omega_starts = [0.1, 0.05, 4, 2, 6, 23] # range of start values for omega
...
#		file_pos = aname.split('.')[0]
		...
		while notconverged:
			...
			if 'check convergence..' in open(file_pos + '_Aamaz_modApos.out').read():
				...
				os.system('mv ' + file_pos + '_Aamaz_modApos.out /pylon1/bi4iflp/mlim/Run_codeml_nudna/nuDNA_codemlout_Aamaz/Aamaz_pos/Aamazout_nu_pos')
				print 'Success!!!', file_pos, ' moved to Aamazout_nu_pos directory'
				print '----------------------------------------------------------------'
				break
...
os.system('grep lnL /pylon1/bi4iflp/mlim/Run_codeml_nudna/nuDNA_codemlout_Aamaz/Aamaz_pos/Aamazout_nu_pos/*pos.out | awk \'{print $1"\t"$5}\' > /pylon1/bi4iflp/mlim/Run_codeml_nudna/nu_Aamaz_lnL_pos.txt')
