# use transcriptometree_noroot_Pmala.tree
...
def ModelA_nullcodeml_Pmala(element, kappastart):
	cml = codeml.Codeml()
	cml.alignment = '/pylon1/bi4iflp/mlim/Run_codeml_mtdna/mtDNA_phylipfiles/' +  element + '.phylip'
	cml.tree = '/pylon1/bi4iflp/mlim/Run_codeml_mtdna/transcriptometree_noroot_Pmala.tree'
	cml.out_file = element + '_Pmala_modAnull.out'
	cml.working_dir = './'
...
for aname in alignmentfiles: 
	if '.phylip' in aname:
		...
		while notconverged:
			...
			ModelA_nullcodeml_Pmala(file_null, kappa_starts[counter])
	
			if 'check convergence..' in open(file_null + '_Pmala_modAnull.out').read(): 
				print 'Nope, try again...', file_null
				print '---------------------------------------'
				
			else: 
				os.system('mv ' + file_null + '_Pmala_modAnull.out /pylon1/bi4iflp/mlim/Run_codeml_mtdna/mtDNA_codemlout_Pmala/Pmalaout')
				print 'Success!!!', file_null, ' moved to Pmalaout directory'
				print '----------------------------------------------------------------'
				break
	...
	if '.phylip' in aname:
		...
		while notconverged:
			...
			if 'check convergence..' in open(file_pos + '_Pmala_modApos.out').read():
				...
				os.system('mv ' + file_pos + '_Pmala_modApos.out /pylon1/bi4iflp/mlim/Run_codeml_mtdna/mtDNA_codemlout_Pmala/Pmalaout')
				print 'Success!!!', file_pos, ' moved to Pmalaout directory'
				print '----------------------------------------------------------------'
				break
...
os.system('grep lnL /pylon1/bi4iflp/mlim/Run_codeml_mtdna/mtDNA_codemlout_Pmala/Pmalaout/*modAnull.out | awk \'{print $1"\t"$5}\' > /pylon1/bi4iflp/mlim/Run_codeml_mtdna/Pmala_lnL_null.txt')
os.system('grep lnL /pylon1/bi4iflp/mlim/Run_codeml_mtdna/mtDNA_codemlout_Pmala/Pmalaout/*pos.out | awk \'{print $1"\t"$5}\' > /pylon1/bi4iflp/mlim/Run_codeml_mtdna/Pmala_lnL_pos.txt')
