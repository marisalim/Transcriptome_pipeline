# use transcriptometree_noroot_Amela.tree
...
def ModelA_nullcodeml_Amela(element, kappastart):
	...
	cml.tree = '/pylon1/bi4iflp/mlim/Run_codeml_mtdna/transcriptometree_noroot_Amela.tree'
...
def ModelA_poscodeml_Amela(element, kappastart, omegastart):
	cml = codeml.Codeml()
	cml.alignment = '/pylon1/bi4iflp/mlim/Run_codeml_mtdna/mtDNA_phylipfiles/' + element + '.phylip'
	cml.tree = '/pylon1/bi4iflp/mlim/Run_codeml_mtdna/transcriptometree_noroot_Amela.tree'
	cml.out_file = element + '_Amela_modApos.out'
	cml.working_dir = './'
	...
	if '.phylip' in aname:
		...
		while notconverged:
			...
			if 'check convergence..' in open(file_null + '_Amela_modAnull.out').read(): 
				...
				os.system('mv ' + file_null + '_Amela_modAnull.out /pylon1/bi4iflp/mlim/Run_codeml_mtdna/mtDNA_codemlout_Amela/Amelaout')
				print 'Success!!!', file_null, ' moved to Amelaout directory'
				print '----------------------------------------------------------------'
				break
...
for aname in alignmentfiles:
	if '.phylip' in aname:
		...
		while notconverged:
			...
			ModelA_poscodeml_Amela(file_pos, kappa_starts[counter], omega_starts[counter])
	
			if 'check convergence..' in open(file_pos + '_Amela_modApos.out').read():
				print 'Nope, try again...', file_pos
				print '---------------------------------------'
				
			else: 
				os.system('mv ' + file_pos + '_Amela_modApos.out /pylon1/bi4iflp/mlim/Run_codeml_mtdna/mtDNA_codemlout_Amela/Amelaout')
				print 'Success!!!', file_pos, ' moved to Amelaout directory'
				print '----------------------------------------------------------------'
				break
...
os.system('grep lnL /pylon1/bi4iflp/mlim/Run_codeml_mtdna/mtDNA_codemlout_Amela/Amelaout/*modAnull.out | awk \'{print $1"\t"$5}\' > /pylon1/bi4iflp/mlim/Run_codeml_mtdna/Amela_lnL_null.txt')
os.system('grep lnL /pylon1/bi4iflp/mlim/Run_codeml_mtdna/mtDNA_codemlout_Amela/Amelaout/*pos.out | awk \'{print $1"\t"$5}\' > /pylon1/bi4iflp/mlim/Run_codeml_mtdna/Amela_lnL_pos.txt')
