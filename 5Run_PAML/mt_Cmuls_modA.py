# use transcriptometree_noroot_Cmuls.tree
...
def ModelA_nullcodeml_Cmuls(element, kappastart):
	cml = codeml.Codeml()
	cml.alignment = '/pylon1/bi4iflp/mlim/Run_codeml_mtdna/mtDNA_phylipfiles/' +  element + '.phylip'
	cml.tree = '/pylon1/bi4iflp/mlim/Run_codeml_mtdna/transcriptometree_noroot_Cmuls.tree'
	cml.out_file = element + '_Cmuls_modAnull.out'
	cml.working_dir = './'
...
def ModelA_poscodeml_Cmuls(element, kappastart, omegastart):
	cml = codeml.Codeml()
	cml.alignment = '/pylon1/bi4iflp/mlim/Run_codeml_mtdna/mtDNA_phylipfiles/' + element + '.phylip'
	cml.tree = '/pylon1/bi4iflp/mlim/Run_codeml_mtdna/transcriptometree_noroot_Cmuls.tree'
	cml.out_file = element + '_Cmuls_modApos.out'
	cml.working_dir = './'
...
for aname in alignmentfiles: 
	if '.phylip' in aname:
		...
		while notconverged:
			...
			ModelA_nullcodeml_Cmuls(file_null, kappa_starts[counter])
	
			if 'check convergence..' in open(file_null + '_Cmuls_modAnull.out').read(): 
				print 'Nope, try again...', file_null
				print '---------------------------------------'
				
			else: 
				os.system('mv ' + file_null + '_Cmuls_modAnull.out /pylon1/bi4iflp/mlim/Run_codeml_mtdna/mtDNA_codemlout_Cmuls/Cmulsout')
				print 'Success!!!', file_null, ' moved to Cmulsout directory'
				print '----------------------------------------------------------------'
				break
...
for aname in alignmentfiles:
	if '.phylip' in aname:
		...
		while notconverged:
			...
			ModelA_poscodeml_Cmuls(file_pos, kappa_starts[counter], omega_starts[counter])
	
			if 'check convergence..' in open(file_pos + '_Cmuls_modApos.out').read():
				print 'Nope, try again...', file_pos
				print '---------------------------------------'
				
			else: 
				os.system('mv ' + file_pos + '_Cmuls_modApos.out /pylon1/bi4iflp/mlim/Run_codeml_mtdna/mtDNA_codemlout_Cmuls/Cmulsout')
				print 'Success!!!', file_pos, ' moved to Cmulsout directory'
				print '----------------------------------------------------------------'
				break
...
os.system('grep lnL /pylon1/bi4iflp/mlim/Run_codeml_mtdna/mtDNA_codemlout_Cmuls/Cmulsout/*modAnull.out | awk \'{print $1"\t"$5}\' > /pylon1/bi4iflp/mlim/Run_codeml_mtdna/Cmuls_lnL_null.txt')
os.system('grep lnL /pylon1/bi4iflp/mlim/Run_codeml_mtdna/mtDNA_codemlout_Cmuls/Cmulsout/*pos.out | awk \'{print $1"\t"$5}\' > /pylon1/bi4iflp/mlim/Run_codeml_mtdna/Cmuls_lnL_pos.txt')
