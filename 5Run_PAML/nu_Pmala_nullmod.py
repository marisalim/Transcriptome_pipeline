...
#		file_null = aname.split('.')[0]
		...
		while notconverged:
			...
			if 'check convergence..' in open(file_null + '_Pmala_modAnull.out').read(): 
				...
				os.system('mv ' + file_null + '_Pmala_modAnull.out /pylon1/bi4iflp/mlim/Run_codeml_nudna/nuDNA_codemlout_Pmala/Pmalaout_nu')
				print 'Success!!!', file_null, ' moved to Pmalaout_nu directory'
				print '----------------------------------------------------------------'
				break
...
os.system('grep lnL /pylon1/bi4iflp/mlim/Run_codeml_nudna/nuDNA_codemlout_Pmala/Pmalaout_nu/*modAnull.out | awk \'{print $1"\t"$5}\' > /pylon1/bi4iflp/mlim/Run_codeml_nudna/nu_Pmala_lnL_null.txt')
