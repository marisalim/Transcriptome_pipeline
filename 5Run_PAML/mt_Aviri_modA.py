# use transcriptometree_noroot_Aviri.tree
...
def ModelA_nullcodeml_Aviri(element, kappastart):
	cml = codeml.Codeml()
	cml.alignment = '/pylon1/bi4iflp/mlim/Run_codeml_mtdna/mtDNA_phylipfiles/' +  element + '.phylip'
	cml.tree = '/pylon1/bi4iflp/mlim/Run_codeml_mtdna/transcriptometree_noroot_Aviri.tree'
	cml.out_file = element + '_Aviri_modAnull.out'
	cml.working_dir = './'
...
for aname in alignmentfiles: 
	if '.phylip' in aname:
		...
		while notconverged:
			...
			ModelA_nullcodeml_Aviri(file_null, kappa_starts[counter])
	
			if 'check convergence..' in open(file_null + '_Aviri_modAnull.out').read(): 
				print 'Nope, try again...', file_null
				print '---------------------------------------'
				
			else: 
				os.system('mv ' + file_null + '_Aviri_modAnull.out /pylon1/bi4iflp/mlim/Run_codeml_mtdna/mtDNA_codemlout_Aviri/Aviriout')
				print 'Success!!!', file_null, ' moved to Aviriout directory'
				print '----------------------------------------------------------------'
				break
	...
	if '.phylip' in aname:
		...
		while notconverged:
			...
			if 'check convergence..' in open(file_pos + '_Aviri_modApos.out').read():
				...
				os.system('mv ' + file_pos + '_Aviri_modApos.out /pylon1/bi4iflp/mlim/Run_codeml_mtdna/mtDNA_codemlout_Aviri/Aviriout')
				print 'Success!!!', file_pos, ' moved to Aviriout directory'
				print '----------------------------------------------------------------'
				break
...
os.system('grep lnL /pylon1/bi4iflp/mlim/Run_codeml_mtdna/mtDNA_codemlout_Aviri/Aviriout/*modAnull.out | awk \'{print $1"\t"$5}\' > /pylon1/bi4iflp/mlim/Run_codeml_mtdna/Aviri_lnL_null.txt')
os.system('grep lnL /pylon1/bi4iflp/mlim/Run_codeml_mtdna/mtDNA_codemlout_Aviri/Aviriout/*pos.out | awk \'{print $1"\t"$5}\' > /pylon1/bi4iflp/mlim/Run_codeml_mtdna/Aviri_lnL_pos.txt')
