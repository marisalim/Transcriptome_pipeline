	...
	if '.phylip' in aname:
		...
		while notconverged:
			...
			if 'check convergence..' in open(file_null + '_Ccoru_modAnull.out').read(): 
				...
				os.system('mv ' + file_null + '_Ccoru_modAnull.out /pylon1/bi4iflp/mlim/Run_codeml_nudna/nuDNA_codemlout_Ccoru/Ccoruout_nu')
				print 'Success!!!', file_null, ' moved to Ccoruout_nu directory'
				print '----------------------------------------------------------------'
				break
...
os.system('grep lnL /pylon1/bi4iflp/mlim/Run_codeml_nudna/nuDNA_codemlout_Ccoru/Ccoruout_nu/*modAnull.out | awk \'{print $1"\t"$5}\' > /pylon1/bi4iflp/mlim/Run_codeml_nudna/nu_Ccoru_lnL_null.txt')
