# use transcriptometree_noroot_Phart.tree
...
def ModelA_nullcodeml_Phart(element, kappastart):
	cml = codeml.Codeml()
	cml.alignment = '/pylon1/bi4iflp/mlim/Run_codeml_mtdna/mtDNA_phylipfiles/' +  element + '.phylip'
	cml.tree = '/pylon1/bi4iflp/mlim/Run_codeml_mtdna/transcriptometree_noroot_Phart.tree'
	cml.out_file = element + '_Phart_modAnull.out'
	cml.working_dir = './'
...
def ModelA_poscodeml_Phart(element, kappastart, omegastart):
	cml = codeml.Codeml()
	cml.alignment = '/pylon1/bi4iflp/mlim/Run_codeml_mtdna/mtDNA_phylipfiles/' + element + '.phylip'
	cml.tree = '/pylon1/bi4iflp/mlim/Run_codeml_mtdna/transcriptometree_noroot_Phart.tree'
	cml.out_file = element + '_Phart_modApos.out'
	cml.working_dir = './'
...
for aname in alignmentfiles: 
	if '.phylip' in aname:
		...
		while notconverged:
			...
			ModelA_nullcodeml_Phart(file_null, kappa_starts[counter])
	
			if 'check convergence..' in open(file_null + '_Phart_modAnull.out').read(): 
				print 'Nope, try again...', file_null
				print '---------------------------------------'
				
			else: 
				os.system('mv ' + file_null + '_Phart_modAnull.out /pylon1/bi4iflp/mlim/Run_codeml_mtdna/mtDNA_codemlout_Phart/Phartout')
				print 'Success!!!', file_null, ' moved to Phartout directory'
				print '----------------------------------------------------------------'
				break
...
for aname in alignmentfiles:
	if '.phylip' in aname:
		...
		while notconverged:
			...
			ModelA_poscodeml_Phart(file_pos, kappa_starts[counter], omega_starts[counter])
	
			if 'check convergence..' in open(file_pos + '_Phart_modApos.out').read():
				print 'Nope, try again...', file_pos
				print '---------------------------------------'
				
			else: 
				os.system('mv ' + file_pos + '_Phart_modApos.out /pylon1/bi4iflp/mlim/Run_codeml_mtdna/mtDNA_codemlout_Phart/Phartout')
				print 'Success!!!', file_pos, ' moved to Phartout directory'
				print '----------------------------------------------------------------'
				break
...
os.system('grep lnL /pylon1/bi4iflp/mlim/Run_codeml_mtdna/mtDNA_codemlout_Phart/Phartout/*modAnull.out | awk \'{print $1"\t"$5}\' > /pylon1/bi4iflp/mlim/Run_codeml_mtdna/Phart_lnL_null.txt')
os.system('grep lnL /pylon1/bi4iflp/mlim/Run_codeml_mtdna/mtDNA_codemlout_Phart/Phartout/*pos.out | awk \'{print $1"\t"$5}\' > /pylon1/bi4iflp/mlim/Run_codeml_mtdna/Phart_lnL_pos.txt')
