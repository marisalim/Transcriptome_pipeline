...
def ModelA_poscodeml_Mphoe(element, kappastart, omegastart):
	cml = codeml.Codeml()
	...
	cml.out_file = element + '_Mphoe_modApos.out'
	cml.working_dir = './'
...
for aname in alignmentfiles_pos: # for reruns
	if '.phylip' in aname:
		...
		while notconverged:
			...
			ModelA_poscodeml_Mphoe(file_pos, kappa_starts[counter], omega_starts[counter])
	
			if 'check convergence..' in open(file_pos + '_Mphoe_modApos.out').read():
				print 'Nope, try again...', file_pos
				print '---------------------------------------'
				
			else: 
				os.system('mv ' + file_pos + '_Mphoe_modApos.out /pylon1/bi4iflp/mlim/Run_codeml_nudna/nuDNA_codemlout_Mphoe/Mphoe_pos/Mphoeout_nu_pos')
				print 'Success!!!', file_pos, ' moved to Mphoeout_nu_pos directory'
				print '----------------------------------------------------------------'
				break
...
os.system('grep lnL /pylon1/bi4iflp/mlim/Run_codeml_nudna/nuDNA_codemlout_Mphoe/Mphoe_pos/Mphoeout_nu_pos/*pos.out | awk \'{print $1"\t"$5}\' > /pylon1/bi4iflp/mlim/Run_codeml_nudna/nu_Mphoe_lnL_pos.txt')
