# use transcriptometree_noroot_Aamaz.tree
...
def ModelA_nullcodeml_Aamaz(element, kappastart):
	cml = codeml.Codeml()
	cml.alignment = '/pylon1/bi4iflp/mlim/Run_codeml_mtdna/mtDNA_phylipfiles/' +  element + '.phylip'
	cml.tree = '/pylon1/bi4iflp/mlim/Run_codeml_mtdna/transcriptometree_noroot_Aamaz.tree'
	cml.out_file = element + '_Aamaz_modAnull.out'
	cml.working_dir = './'
...
def ModelA_poscodeml_Aamaz(element, kappastart, omegastart):
	cml = codeml.Codeml()
	cml.alignment = '/pylon1/bi4iflp/mlim/Run_codeml_mtdna/mtDNA_phylipfiles/' + element + '.phylip'
	cml.tree = '/pylon1/bi4iflp/mlim/Run_codeml_mtdna/transcriptometree_noroot_Aamaz.tree'
	cml.out_file = element + '_Aamaz_modApos.out'
	cml.working_dir = './'
...
for aname in alignmentfiles: 
	if '.phylip' in aname:
		...
		while notconverged:
			...
			ModelA_nullcodeml_Aamaz(file_null, kappa_starts[counter])
	
			if 'check convergence..' in open(file_null + '_Aamaz_modAnull.out').read(): 
				print 'Nope, try again...', file_null
				print '---------------------------------------'
				
			else: 
				os.system('mv ' + file_null + '_Aamaz_modAnull.out /pylon1/bi4iflp/mlim/Run_codeml_mtdna/mtDNA_codemlout_Aamaz/Aamazout')
				print 'Success!!!', file_null, ' moved to Aamazout directory'
				print '----------------------------------------------------------------'
				break
...
for aname in alignmentfiles:
	if '.phylip' in aname:
		...
		while notconverged:
			...
			ModelA_poscodeml_Aamaz(file_pos, kappa_starts[counter], omega_starts[counter])
	
			if 'check convergence..' in open(file_pos + '_Aamaz_modApos.out').read():
				print 'Nope, try again...', file_pos
				print '---------------------------------------'
				
			else: 
				os.system('mv ' + file_pos + '_Aamaz_modApos.out /pylon1/bi4iflp/mlim/Run_codeml_mtdna/mtDNA_codemlout_Aamaz/Aamazout')
				print 'Success!!!', file_pos, ' moved to Aamazout directory'
				print '----------------------------------------------------------------'
				break
...
os.system('grep lnL /pylon1/bi4iflp/mlim/Run_codeml_mtdna/mtDNA_codemlout_Aamaz/Aamazout/*modAnull.out | awk \'{print $1"\t"$5}\' > /pylon1/bi4iflp/mlim/Run_codeml_mtdna/Aamaz_lnL_null.txt')
os.system('grep lnL /pylon1/bi4iflp/mlim/Run_codeml_mtdna/mtDNA_codemlout_Aamaz/Aamazout/*pos.out | awk \'{print $1"\t"$5}\' > /pylon1/bi4iflp/mlim/Run_codeml_mtdna/Aamaz_lnL_pos.txt')
