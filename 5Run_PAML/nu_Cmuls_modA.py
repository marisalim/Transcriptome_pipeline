...
def ModelA_poscodeml_Cmuls(element, kappastart, omegastart):
	...
	cml.tree = '/pylon1/bi4iflp/mlim/Run_codeml_nudna/transcriptometree_noroot_Cmuls.tree' #Note: using an unrooted tree
...
#		file_pos = aname.split('.')[0]
		...
		while notconverged:
			...
			if 'check convergence..' in open(file_pos + '_Cmuls_modApos.out').read():
				...
				os.system('mv ' + file_pos + '_Cmuls_modApos.out /pylon1/bi4iflp/mlim/Run_codeml_nudna/nuDNA_codemlout_Cmuls/Cmuls_pos/Cmulsout_nu_pos')
				print 'Success!!!', file_pos, ' moved to Cmulsout_nu_pos directory'
				print '----------------------------------------------------------------'
				break
...
os.system('grep lnL /pylon1/bi4iflp/mlim/Run_codeml_nudna/nuDNA_codemlout_Cmuls/Cmuls_pos/Cmulsout_nu_pos/*pos.out | awk \'{print $1"\t"$5}\' > /pylon1/bi4iflp/mlim/Run_codeml_nudna/nu_Cmuls_lnL_pos.txt')
