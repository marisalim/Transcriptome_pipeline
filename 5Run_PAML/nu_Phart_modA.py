...
def ModelA_poscodeml_Phart(element, kappastart, omegastart):
	...
	cml.tree = '/pylon1/bi4iflp/mlim/Run_codeml_nudna/transcriptometree_noroot_Phart.tree' #Note: using an unrooted tree
...
# FOR RERUNS:
alignmentfiles_pos = ['/pylon1/bi4iflp/mlim/Run_codeml_nudna/phylipforpaml/ENSTGUP00000006101.phylip']

# Choose start values
kappa_starts = [0, 0.005, 0.2, 3, 8, 15] # range of start values for kappa
#kappa_starts = [0,0.5] # use to test code
omega_starts = [0, 0.005, 0.2, 3, 8, 15] # range of start values for omega
...
#		file_pos = aname.split('.')[0]
		...
		while notconverged:
			...
			if 'check convergence..' in open(file_pos + '_Phart_modApos.out').read():
				...
				os.system('mv ' + file_pos + '_Phart_modApos.out /pylon1/bi4iflp/mlim/Run_codeml_nudna/nuDNA_codemlout_Phart/Phart_pos/Phartout_nu_pos')
				print 'Success!!!', file_pos, ' moved to Phartout_nu_pos directory'
				print '----------------------------------------------------------------'
				break
...
os.system('grep lnL /pylon1/bi4iflp/mlim/Run_codeml_nudna/nuDNA_codemlout_Phart/Phart_pos/Phartout_nu_pos/*pos.out | awk \'{print $1"\t"$5}\' > /pylon1/bi4iflp/mlim/Run_codeml_nudna/nu_Phart_lnL_pos.txt')
