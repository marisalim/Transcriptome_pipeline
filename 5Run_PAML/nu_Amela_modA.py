...
# Choose start values
kappa_starts = [0, 0.01, 0.1, 1, 5, 8] # range of start values for kappa
#kappa_starts = [0,0.5] # use to test code
omega_starts = [0, 0.01, 0.1, 1, 5, 8] # range of start values for omega
...
#		file_pos = aname.split('.')[0]
		...
		while notconverged:
			...
			if 'check convergence..' in open(file_pos + '_Amela_modApos.out').read():
				...
				os.system('mv ' + file_pos + '_Amela_modApos.out /pylon1/bi4iflp/mlim/Run_codeml_nudna/nuDNA_codemlout_Amela/Amela_pos/Amelaout_nu_pos')
				print 'Success!!!', file_pos, ' moved to Amelaout_nu_pos directory'
				print '----------------------------------------------------------------'
				break
...
os.system('grep lnL /pylon1/bi4iflp/mlim/Run_codeml_nudna/nuDNA_codemlout_Amela/Amela_pos/Amelaout_nu_pos/*pos.out | awk \'{print $1"\t"$5}\' > /pylon1/bi4iflp/mlim/Run_codeml_nudna/nu_Amela_lnL_pos.txt')
