...
#FOR RERUNS:
alignmentfiles_null = ['/pylon1/bi4iflp/mlim/Run_codeml_nudna/phylipforpaml/ENSTGUP00000003641.phylip']

# Choose start values
kappa_starts = [0, 0.001, 2, 3.5, 8, 12] # range of start values for kappa
...
#		file_null = aname.split('.')[0]
		...
		while notconverged:
			...
			if 'check convergence..' in open(file_null + '_Phart_modAnull.out').read(): 
				...
				os.system('mv ' + file_null + '_Phart_modAnull.out /pylon1/bi4iflp/mlim/Run_codeml_nudna/nuDNA_codemlout_Phart/Phartout_nu')
				print 'Success!!!', file_null, ' moved to Phartout_nu directory'
				print '----------------------------------------------------------------'
				break
...
os.system('grep lnL /pylon1/bi4iflp/mlim/Run_codeml_nudna/nuDNA_codemlout_Phart/Phartout_nu/*modAnull.out | awk \'{print $1"\t"$5}\' > /pylon1/bi4iflp/mlim/Run_codeml_nudna/nu_Phart_lnL_null.txt')
